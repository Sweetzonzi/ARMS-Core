...
3. 注入离散事件（来自 pendingEvents）
   ...
   if (pendingEvents.contains(KNOCKDOWN))    logic.broadcastEvent("knockdown")

4. 逻辑层推进
   logic.progress()  // 递归驱动 posture 顶层 + 活跃子控（gait/vert）
   → 写入 variables: POSTURE / GAIT / VERTICAL 枚举
   → MOVE_SPEED_MODIFIER（KCC 直接读取）
   → GAIT_CAN_* / VERTICAL_CAN_* 来源许可，再合并为 CAN_MOVE / CAN_JUMP
...
子机产出 `GAIT` / `VERTICAL` 枚举、`MOVE_SPEED_MODIFIER` 浮点及各自的 `GAIT_CAN_*` / `VERTICAL_CAN_*` 输入许可。`MechaLogicStateMachine.progress()` 在状态树推进后结合 posture 合并为最终 `CAN_MOVE` / `CAN_JUMP`，既保留来源信息，也给 KCC 提供统一判断结果。
...
**转移条件**：实际转移条件以代码中的 `SimpleVariableCondition` 工厂方法为准（`isTrue(HAS_INPUT)`、`isFalse(WALK_KEY_DOWN)` 等）。`idle ↔ drift` 使用 0.05 / 0.15 m/s 的滞回阈值；stun / hard_land 退出仍保留计时器条件 TODO。
...
```
ground ──(KCC_JUMP_CHARGING)──▶ jump_charge
jump_charge ──(!KCC_JUMP_CHARGING)──▶ ground
```

KCC 是跳跃蓄力的权威状态源，vertical 子机在下一物理步镜像它。ground 允许 vertical 移动和开始跳跃；jump_charge 禁止 vertical 移动和重复开始跳跃，但 `MechaControl` 会继续向已经蓄力的 KCC 透传按住/松开信号。离地后 posture 切 air，stand_vert 子机自然停用。
...
| 输入许可 | `GAIT_CAN_MOVE` / `GAIT_CAN_JUMP` | boolean | gait 子机的许可来源 |
| 输入许可 | `VERTICAL_CAN_MOVE` / `VERTICAL_CAN_JUMP` | boolean | vertical 子机的许可来源 |
